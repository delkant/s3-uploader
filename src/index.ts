...
const DELETE_FILE_AFTER_UPLOADED = true;
const sts = new STS();
...
async function getBucketNames() {
  ...
  const buckets =
    Buckets != null
      ? (Buckets.filter((bucket) => bucket != null && bucket.Name != null).map(
          (b) => b.Name
        ) as string[])
      : null;
...
async function createBucket({
  ...
  if (!name) {
    ...
    try {
      console.log(
        c.green(`Creating bucket ${bucketName} in region: ${region}`)
      );
      await s3
        .createBucket({
          Bucket: bucketName,
    ...
    console.error(
      "must be a valid bucket name -> https://docs.aws.amazon.com/AmazonS3/latest/dev/BucketRestrictions.html"
    );
...
async function makeDir(dirName?: string): Promise<string> {
  if (dirName == null) {
    const { dir } = await inquirer.prompt([
      {
        ...
        source: async (answersSoFar: any, input: any) => {
          ...
          const dirs = possibleMatches.filter((file) => {
    ...
    if (!stat.isDirectory() && !stat.isFile()) {
      console.error(
        `Path ${dirName} doesn't exist, please choose one which does or create one`
      );
...
async function renderProgress({
  ...
  Object.keys(files).forEach((key) => {
    ...
    if (DELETE_FILE_AFTER_UPLOADED && files[key] === "100%") {
      deleteFile(`${dir}/${key}`);
    }
  });
}
...
async function run() {
  ...
  const options: { bucket?: string; dir?: string; region?: string } =
    program.opts();
  let region: string | undefined = options.region;
  if (region == null) {
    console.log(
      c.yellow(
        "Warning: Missing --region argument, defaulting to AWS_REGION env variable"
      )
    );
  }
  ...
  if (region != null) {
    ...
    console.log(
      c.red(
        "Error: Missing AWS_REGION env variable as well as --region argument, cannot set region"
      )
    );
    return;
  }
